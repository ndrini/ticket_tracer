...
def carica_scontrino(cursore, dati):
    """
...
def main(argv):
    ...
    percorsi = sorted(glob.glob(os.path.join(DIR_STRUTTURATI, "*.json")))
    if limite:
        percorsi = percorsi[:limite]
    if not percorsi:
        print(f"Nessuno scontrino in {DIR_STRUTTURATI}/. Esegui prima la Fase C.")
        return 1

    os.makedirs(os.path.dirname(db) or ".", exist_ok=True)
    init_db(db)

    print(f"Fase D — {len(percorsi)} scontrini -> {db}\n")
    ...
    try:
        for percorso in percorsi:
            with open(percorso) as fh:
                dati = json.load(fh)
            esito = carica_scontrino(cursore, dati)
