#!/usr/bin/env python3
"""
Week 1, Task 1.5 FIXED: Test validators su 80 scontrini

Corretto: LLaVA accuracy non significa "prendi i primi N items"
ma "accuracy complessiva sull'estrazione dell'intero scontrino".

Simulazione realistica:
- Per ogni item nel baseline, 74% prob di estarre correttamente
- Accuracy mean = 74% significa: su 100 items, 74 sono corretti
- Per 80 scontrini, this is reflected nel confidence score
"""
...
print("\n" + "=" * 70)
print("Week 1, Task 1.5 FIXED: Test Validators su 80 Scontrini")
print("=" * 70)
...
# Test validators
print("\n🔄 Simulazione LLaVA + Testing Validators (corretta)...")
...
for idx, sample in enumerate(samples, 1):
    ...
    # Realistic LLaVA simulation:
    # For each item in baseline, apply accuracy independently
    baseline_items = baseline["items"].copy()
    random.seed(idx)  # Deterministic but varied

    simulated_items = []
    for item in baseline_items:
        # 74% chance to extract correctly
        if random.random() < accuracy_mean:
            simulated_items.append(item.copy())

    has_hallucination = False
    if hallucination_count > 0:
        # Add hallucinated item
        fake_item = {
    ...
    per_receipt_stats[sha256] = {
        ...
        "extracted_items": len(simulated_items),
...
print(f"     Baseline: {hallucination_base_count} scontrini")
print(f"     Flagged: {hallucination_detected}")
if hallucination_base_count > 0:
    detection_rate = 100 * hallucination_detected / hallucination_base_count
    print(f"     Detection rate: {detection_rate:.1f}%")

print(f"\n   False Positive Rate:")
valid_count = validation_stats["valid_not_flagged"] + validation_stats["valid_flagged"]
print(f"     Valid scontrini: {valid_count}")
print(f"     Erroneously flagged: {validation_stats['valid_flagged']}")
if valid_count > 0:
    fp_rate = 100 * validation_stats['valid_flagged'] / valid_count
...
print(f"     Valid but flagged: {validation_stats['valid_flagged']} ({100*validation_stats['valid_flagged']/total_validated:.1f}%)")
print(f"     Hallucinated + flagged: {hallucination_detected} ({100*hallucination_detected/total_validated:.1f}%)")
print(f"     Hallucinated but not flagged: {hallucination_base_count - hallucination_detected} ({100*(hallucination_base_count - hallucination_detected)/total_validated:.1f}%)")
...
analysis = {
    ...
    "false_positive_rate": 100 * validation_stats['valid_flagged'] / valid_count if valid_count > 0 else 0,
...
if detection_rate >= 80 and fp_rate < 3 and manual_review_rate < 8:
    ...
    print(f"\n⚠️  VALIDATORS ACCEPTABLE WITH TUNING")
    ...
    print(f"\n❌ VALIDATORS NEED TUNING")
    print(f"❌ Detects {detection_rate:.1f}% of hallucinations (too low)")
    print(f"❌ False positive rate {fp_rate:.1f}% (too high)")
    print(f"→ Adjust validator rules and re-test before Week 2")

conn.close()

print("\n✅ Task 1.5 FIXED Completato")
print("=" * 70)
